...
2. Create a `database.csv` file. Add your data to this file. Please refer to the `example_database.csv` for appropriate column names. Note that this repo is currently set up for botanical (plant) flashcards, so you'll find columns like 'genus' and 'species'. When you make changes to the column names, make sure to update the corresponding references in the `flashcard_creator.py` module.
